...
#----------------------------------------------------------------------------------------
def vae_loss_function_(recon_x: torch.Tensor, x: torch.Tensor, mu: torch.Tensor,
    ...
    Args:
        ...
        reduction: Specifies the reduction method for the reconstruction loss Default:'mean' (average loss over batch and elements) or 'sum' (summed loss)  Default: 'mean'.
        rec_loss_type: reconstruction loss type: BCE, L1, MSE, MSE+L1

    Returns:
        dict[str, torch.Tensor]: A dictionary containing:
            - 'total_loss': Weighted sum of reconstruction and KLD losses.
            - 'recon_loss': Reconstruction error/loss (MSE or BCE:binary_cross_entropy)
            - 'kld_loss': Analytical KL Divergence loss against a standard normal prior.

    Raises:
        ValueError: If `reduction` is not 'mean' or 'sum', or Re-raises any exception caught during processing after logging.
    """
    try:
        if reduction not in ("mean", "sum"):
            raise ValueError(f"Invalid reduction mode: '{reduction}'. Supported modes: 'mean', 'sum'.")

        # Reconstruction Loss (MSE or BCE, L1, MSEL1)
        if rec_loss_type == 'BCE':
            recon_loss = torch.nn.functional.binary_cross_entropy(recon_x, x, reduction=reduction) # only when data [0,1]
            ...
            l1_weight = 0.5  # Adjust between 0.0 and 1.0 depending on desired sharpness
